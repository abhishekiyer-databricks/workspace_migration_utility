    ...
    def _remap_compute(self, settings: dict, warnings: list) -> None:
        ...
        for job_cluster in settings.get("job_clusters") or []:
            if isinstance(job_cluster, dict):
                self._remap_new_cluster(job_cluster.get("new_cluster"), warnings)

        for task in settings.get("tasks") or []:
            if not isinstance(task, dict):
                continue
            self._remap_new_cluster(task.get("new_cluster"), warnings)
            src_id = safe_str(task.get("existing_cluster_id"))
            if not src_id:
                continue
            target_id, key = self.remap_id("cluster", src_id)
            if target_id:
                task["existing_cluster_id"] = target_id
            else:
                # Leaving a source cluster id would make the job fail at RUN time with an opaque
                # error. Naming it here turns that into a report line.
                warnings.append(
                    f"task {safe_str(task.get('task_key'))!r} references source cluster {src_id!r}"
                    + (f" ({key!r})" if key else "")
                    + " which has no target equivalent — the job was created but will FAIL AT RUN "
                      "until compute is imported and the job re-imported "
                      "(retry_mode=failed_only).")

    def _remap_new_cluster(self, new_cluster, warnings: list) -> None:
        """Remap a `new_cluster` spec's policy + pool ids, and drop node types when pooled."""
        ...
        for field, ref_type in (("policy_id", "cluster_policy"),
            ...
            if target_id:
                new_cluster[field] = target_id
            else:
                new_cluster.pop(field, None)
                warnings.append(
                    f"a job cluster's {field} pointed at source {ref_type} {src_id!r}"
                    + (f" ({key!r})" if key else "")
                    + " with no target equivalent, so the reference was dropped to let the job be "
                      "created; the cluster will not carry that policy/pool.")
